...
const gridListStyles = tv({
  base: "relative [&>[data-drop-target]]:border [&>[data-drop-target]]:border-primary [&::-webkit-scrollbar]:size-0.5 [scrollbar-width:thin] max-h-96  rounded-lg"
})

const GridListSubtasks = <T extends object>({ children, className, ...props }: GridListProps<T>) => (
...
const itemStyles = tv({
  base: "relative group transition outline-none flex cursor-default select-none my-2 gap-3 lg:text-sm text-fg -outline-offset-2 first:rounded-t-md first:border-t-0 last:mb-0 last:rounded-b-md last:border-b-0",
  variants: {
...
GridListSubtasks.Item = Item
GridListSubtasks.EmptyState = EmptyState

export { GridListSubtasks }
